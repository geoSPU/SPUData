...
subtitle: |
  ...
  10154.129486/2020-19  
author: "Luiz Fernando Palin Droubi"
date: "`r as.Date('2020-10-15')`"
...
```{r}
nup <- "10154-129486-2020-19"
# 1. Digitação da Geometria
#  
coords <- 
  matrix(c(734447.093, 6898448.139, #V1
           734402.093, 6898359.139, #V2
           734312.093, 6898403.139, #V3
           734357.093, 6898493.139, #V4
           734447.093, 6898448.139),#V1
         ncol = 2, byrow = TRUE)
#
# 2. Criação da feição  
#
...
attr <- data.frame(
  ...
  tipo = "aquatica",
  rip = NA,
  interessado = "Denis Carlos dos Santos",
  area = st_area(spl),
  area_uniao = st_area(spl),
  nup = nup,
  protocolo = "Ofício 10388228 (7491863)",
  ref = 10247233,
  municipio = 8113,
  logradouro = "14",
  trecho = "00",
  aval = set_units(815.63, Reais/m^2)*st_area(spl),
  dataaval = as.Date("2021-02-21"))
#
...
m <- spl_df %>%
  ...
  addPolygons(stroke = TRUE, weight = 2, color = "white", fillColor = "red",
            dashArray = "10 5", group = "Em análise") %>%
  addPolygons(data = polUniao, color = "orange", group = "Terras da União") %>%
  addPolygons(data = entregas, color = "blue", group = "Áreas entregues") %>% 
  ...
  addLegend(labels = "Terras de Marinha", colors = "orange",
            group = "Terras da União", position = "topright") %>%
  addLegend(labels = "Em análise", colors = "red",
            group = "Em análise", position = "topright") %>%
  addLegend(labels = "Áreas entregues", colors = "blue", 
            group = "Áreas entregues", position = "topright") %>% 
